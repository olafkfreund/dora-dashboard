...
const base: Omit<Metric, "accent" | "sourceDetail">[] = [
  ...
  },
  {
    ...
    unit: "days",
    definition:
      "Time from when an issue is created until it is resolved (request → done). This matches how the delivery teams and their Jira board report cycle time; the code-side GitLab DORA Lead Time (commit → deploy) is tracked separately.",
    formula: "median(resolved − created) for completed items (Stories/Tasks/Bugs — excludes sub-tasks). Per-PI breakdown counts each issue in every Programme Increment it belongs to.",
    insight:
...
const SOURCE_DETAIL: Record<string, string> = {
  ...
  "cycle-time": "Jira — median of (resolved − created) across issues completed in the window. The drill-down breaks it down per Programme Increment across all history, counting each issue in every PI it belongs to (PI is a multi-value field).",
  ...
  "delivery-predictability": "Jira — completed vs committed story points per Program Increment (P1–P6); an issue spanning multiple PIs counts toward each.",
  "average-velocity": "Jira — mean completed story points per Program Increment (P1–P6); needs story-pointed issues. An issue in multiple PIs counts toward each.",
