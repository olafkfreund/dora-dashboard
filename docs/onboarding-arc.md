...
Step-by-step record of how `dora-dashboard` was onboarded into the ARC portal —
including live **ServiceNow Change Request** and **CMDB** integration — so the
next service can follow the same recipe.
...
1. **One name, used verbatim everywhere:**
   ...
   For this service that string is **`dora-dashboard`**.
...
> The GitHub **repo** can be named differently (here `synechron/DORA`) — that only
> links pipelines/issues. The **service** name is `dora-dashboard` and is what all
> the data surfaces key on.

## What this service is (context)

- Next.js app, deployed by **Flux** (`clusters/aws-dashboard/helmrelease.yaml`) to
  namespace **`dora`** on cluster **`aws-dashboard-cluster`** (AWS).
- CI already emits most of the compliance chain: Trivy scan (hard gate), cosign
  keyless signing, CycloneDX SBOM + attest, and **Fides** trail
  (`FIDES_SERVER_URL` / `FIDES_API_TOKEN` / `FIDES_FLOW_ID`, `fides verify-image`).
- What it was **missing** for full ARC integration: the ARC service registration
  and the **ServiceNow CR + CMDB** steps.
...
| Field | Value | Why |
|---|---|---|
| `slug` | `dora-dashboard` | the join key for deployments/vulns/SBOM everywhere |
| `displayName` | `DORA Dashboard` | catalog label |
| `kubernetesWorkloads` | `[{cloud:aws, cluster:aws-dashboard-cluster, namespace:dora, name:dora-dashboard}]` | links the service to live pods so **topology** resolves; `name` must match the Deployment |
| `githubOwner`/`githubRepo` | `synechron` / `DORA` | links pipelines + issues |
| `fidesFlow` | `dora-dashboard` | which Fides trail the service attests into |
| `criticality` / `tier` | `high` / `internal` | scorecards + graph |
| `businessServiceSysId` | *(set in Step C)* | pointer to the ServiceNow CMDB CI |

**Also add a Git Repo** (Settings → **Git Repos**) for `github:synechron/DORA` so
the pipelines + Issue Sync fan-out includes it. (ARC's `getTenantRepos()` resolves
it; falls back to the tenant's primary repo only if no row exists.)

After Step A the service shows in the **catalog**, **service graph**, and once its
pods are read via the cluster IRSA role, on the **topology** page
(select `aws-dashboard-cluster` in the cluster picker).
...
- **`DeploymentRecord`** (feeds DORA: deployment frequency, lead time,
  change-failure rate) — `service=dora-dashboard`, `provider=github`,
  `environment`, `commitSha`, `crNumber`, `status` (mostly `SUCCESS`, one `FAILED`
  for change-failure rate).
- **`VulnerabilityRecord`** — `service=dora-dashboard`, severity/status/scanType
  (mirrors what the repo's Trivy scan finds).
- **`SbomRecord`** — `service=dora-dashboard`, `format=CycloneDX` (mirrors the
...
1. **CMDB CI** — create/upsert a `cmdb_ci_service` whose **`name` is
   `dora-dashboard`** (must equal the service slug so the CR can anchor to it).
...
2. **Change Request** — create a `change_request` with:
   ...
   - `u_services_deployed` = `dora-dashboard`,
...
Concrete result for `dora-dashboard`: CI `dora-dashboard` (`cmdb_ci_service`) and
CR **CHG0032932** (approved, `u_commit_sha` = the prod deploy commit), linked to the
...
chain on every deploy. Add to `.github/workflows/deploy.yml` (the Fides steps
already exist):

1. **Deployment record → ARC**: after deploy, `POST` to ARC's deployment webhook
   with `service=dora-dashboard`, the `commitSha`, `environment`, `status`, and the
   ...
   repo name (`DORA`), which would not match the slug `dora-dashboard`.
2. **ServiceNow CR**: in the attest phase (after Fides so risk scoring works), call
   ...
   `SERVICES_DEPLOYED=dora-dashboard` and `CI_COMMIT_SHA` set. It stamps
